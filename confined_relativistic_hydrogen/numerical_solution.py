...
# tuples into a list of separate R values (multiples of the bohr radius) and E (bind) values to be used for fitting.
...
"""
Figures 2, 3, and 4 look like each individual plot is the same as in the other figures, but they are not. If you plot all 9 
polynomials (with the data points), you will see a slight difference in radial value and energy value. This is hard to see 
from three separate plots, but you can go ahead and plot all 9 on the same graph to check it out yourself.
...
will use the same R values as they do.
"""
...
"""
For figure 5, you will notice that there is very little difference in energy for large confining radii. It is only when R
gets to about 3*bohr radius that we start to see large differences in energy, hence the need for a comparison.
"""
